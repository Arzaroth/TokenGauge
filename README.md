...
- **Omarchy 4 bar widget**: native Quickshell plugin for the bar that replaced Waybar in Omarchy 4 - brand-marked hero with the plan, provider chips, usage meters with reset + pace, ccusage cost rows with the burn rate, tokens by day, tokens by model with an input/output/cache hover breakdown, an update banner, and a settings pane for the provider toggles and pin-to-bar. Uses omarchy's own UI components, so it follows the active theme. Shares the same config, cache, and daemon as the Waybar module. Install with `scripts/install-omarchy.sh`; a monthly `tokengauge-omarchy-watch` timer reports upstream changes to the shell internals it depends on.
