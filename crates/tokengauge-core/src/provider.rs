...
/// A JWT's payload claims. `None` for an opaque or malformed token, which is
/// never treated as a bad credential on its own - two providers now read a
/// claim off a token that may legitimately not be a JWT at all.
