...
- The popover's "updated" stamp reports when the data was actually fetched, taken from the cache's mtime, and shows the date when the fetch wasn't today. It rendered the current time, so it always claimed the data was fresh even when the fetch behind it was hours old or failing.
